import { classifyPii } from "@/lib/pii";
...
export function processOcrForDetections(
  ...
  ocrResult.lines.forEach((line, lineIdx) => {
    ...
    console.log(`Full line classification:`, lineClassification);
    ...
    if (lineClassification.type !== "other" && isHighQualityDetection(lineText, lineClassification.type) && !skipLineDetection) {
      ...
      if (avgConfidence >= minConfidence) {
        console.log(`✅ Adding line detection: ${lineClassification.type} - "${lineText}"`);
        detections.push({
      ...
      console.log(`⏭️ Skipping line detection for email line: "${lineText}"`);
    }
    ...
    piiMatches.forEach((match, idx) => {
      console.log(`✅ Adding specific detection: ${match.type} - "${match.text}"`);
      detections.push({
...
function findSpecificPiiInLine(
  ...
  console.log(`Searching in cleaned text: "${lineText}"`);
  ...
  const patterns: Array<{ type: PiiType; regex: RegExp; clean: (s: string) => string }> = [
    { 
      ...
      regex: /([a-zA-Z0-9._%-]+\s*@\s*[a-zA-Z0-9.-]+\s*\.\s*[a-zA-Z]{2,})/gi,
      clean: (match: string) => match.replace(/\s+/g, "")
    },
    { 
      type: "url", 
      // Handles: "https://example.com", "https : // example . com", etc.
      regex: /((?:https?\s*:\s*\/\s*\/\s*)?[a-zA-Z0-9-]+\s*\.\s*[a-zA-Z0-9.-]+(?:\s*\/[^\s]*)?)/gi,
      clean: (match: string) => match.replace(/\s+/g, "")
    },
    { 
      type: "ipv4", 
  ...
  patterns.forEach(pattern => {
    ...
    while ((match = pattern.regex.exec(lineText)) !== null) {
      ...
      console.log(`Found ${pattern.type} candidate: "${rawMatch}" -> "${cleanedMatch}"`);
      ...
      if (isOcrFriendlyMatch(cleanedMatch, pattern.type)) {
        ...
        if (matchWords.length > 0) {
          const bbox: BBox = {
            x0: Math.min(...matchWords.map(w => w.bbox.x)) * scaleX,
            y0: Math.min(...matchWords.map(w => w.bbox.y)) * scaleY,
            x1: Math.max(...matchWords.map(w => w.bbox.x + w.bbox.w)) * scaleX,
            y1: Math.max(...matchWords.map(w => w.bbox.y + w.bbox.h)) * scaleY,
          };
          
          const avgConfidence = matchWords.reduce((sum, w) => sum + w.conf, 0) / matchWords.length;
          
          if (avgConfidence >= minConfidence || isLowConfidenceButValid(cleanedMatch, pattern.type, avgConfidence)) {
            console.log(`✅ Validated ${pattern.type}: "${cleanedMatch}"`);
            matches.push({
              type: pattern.type,
              text: cleanedMatch,
              bbox,
              confidence: Math.max(avgConfidence, 50) // Boost confidence for valid matches
            });
          } else {
            console.log(`❌ ${pattern.type} confidence too low: ${avgConfidence}% < ${minConfidence}%`);
          }
        }
      } else {
        console.log(`❌ ${pattern.type} failed validation: "${cleanedMatch}"`);
      }
    }
  });
...
function isLowConfidenceButValid(text: string, type: string, _confidence: number): boolean {
  switch (type) {
    case "url":
...
function isOcrFriendlyMatch(text: string, type: string): boolean {
  ...
  switch (type) {
    case "email":
      ...
      const hasValidStructure = hasAt && hasDot && isLongEnough && !text.startsWith("@");
      console.log(`Email validation: @=${hasAt}, .=${hasDot}, len>5=${isLongEnough}, structure=${hasValidStructure}`);
      return hasValidStructure;
      
    case "url":
      const hasHttp = text.toLowerCase().includes("http");
      const hasValidDot = text.includes(".") && text.length >= 4;
      const isValidUrl = hasHttp || hasValidDot;
      // More permissive - only exclude if it contains @ (likely email)
      const notEmail = !text.includes("@");
      // Special case: allow example.com, google.com etc even in email context
      const isCommonDomain = /\.(com|org|net|edu|gov)$/i.test(text);
      const shouldAllow = notEmail || isCommonDomain;
      console.log(`URL validation: http=${hasHttp}, dot=${hasValidDot}, valid=${isValidUrl}, notEmail=${notEmail}, isCommon=${isCommonDomain}, allow=${shouldAllow}`);
      return isValidUrl && shouldAllow;
      
    case "ipv4":
      const ipParts = text.split(".");
      const has4Parts = ipParts.length === 4;
      const allValidParts = ipParts.every(part => /^\d{1,3}$/.test(part) && parseInt(part) <= 255);
      console.log(`IP validation: 4parts=${has4Parts}, validParts=${allValidParts}, parts=[${ipParts.join(',')}]`);
      return has4Parts && allValidParts;
      
    case "phone":
      const digits = text.replace(/\D/g, "");
      const validLength = digits.length >= 10 && digits.length <= 15;
      console.log(`Phone validation: digits="${digits}", len=${digits.length}, valid=${validLength}`);
      return validLength;
      
    case "cc": // Changed from "credit_card"
      const ccDigits = text.replace(/\D/g, "");
      const validCcLength = ccDigits.length >= 13 && ccDigits.length <= 19;
      console.log(`CC validation: digits="${ccDigits}", len=${ccDigits.length}, valid=${validCcLength}`);
