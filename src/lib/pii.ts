...
export function isEmail(s: string): boolean {
  ...
  const normalized = s
    ...
    .replace(/[0O]/g, match => match === '0' ? 'O' : '0') // Sometimes O and 0 get confused
    .replace(/[1l|]/g, 'l') // OCR often confuses 1, l, |
    .trim();
...
export function isUrl(s: string): boolean {
  ...
  const normalized = s
    ...
    // Handle common protocol issues
    .replace(/^h[t1l|]+p[s5]?[:;]\/\//, 'http://')
    .replace(/^[w\s]*[w\s]*[w\s]*\./, 'www.')
    .trim();
